...
- 動的参照関係
	...
	- 導出型関数を基礎とする項目は、「物理的に保持すべきではない」
		- 「他の項目を使って必要に応じて導出できる」から
		- 「導出して得られるはずの値と矛盾する値が書き込まれる可能性があるから」

	- 導出型関数を基礎とする項目はデータモデル上で扱わないのか？
		- 論理フィールドとして扱う
	
	- 論理フィールドは重要
		- 「論理フィールドがそのモデルの最終的な目的を表す場合がある」から(第1章)
		- 「論理フィールドを用いて参照関係を構成することで、テーブル間の関係が俄然豊かになる」から
			- これが動的参照関係(図3-6,3-7)

	- 論理フィールドはDB上でどう表すのか？
		- 表せない
		- 「良くも悪くもデータモデルはRDBを超えています」

- 正規化のための3つのルール

	- 一般的な「正規化」のことはここでは扱わない
	- 筆者オリジナルの正規化法を説明する
		- 入れ子構造の排除
		- 不要な関数従属性の排除
		- 有効な識別子の網羅
	...
	- 入れ子構造の排除
		- 「フィールドは単一のデータを保持するものでなければいけません」
		- 図3-8
		- 商品コードに意味を持たせることはNGなのか？
			- そうではない
			- ユーザの利便性にとっては意味がある
			- 「しかしその場合でも、桁を区切って内部構造を取り出して処理するようなアプリを設計すべきではありません」
			- 「なぜならその種のコード体系は、遅かれ早かれ破綻するから」
			- 「繰り返し回数がじゅうぶんに少なければ、テーブル上の繰り返し構造は許容される」「『程度問題』」
		
	- 不要な関数従属性の排除
		- 前出
	
	- 有効な識別子の網羅
		- 「関数従属性を伴わない定義域(ドメイン)に関する制約(ドメイン制約)に注意をうながすためのルール」
		- 図3-9,3-10
		- 下記に注意喚起するためのルール
			- 「『有効なドメイン制約に対してテーブル(主キー)が用意されているか』」
			- 「『無意味なドメイン制約を規定するテーブル(主キー)が盛り込まれていないか』」

ーーー9/19ここからーーーーーーーーーー

- 正規化崩しの3パターン

	- 正規化崩しとは？
		- 「いったん確立された正規形をあえて崩すこと」
		- 「レスポンス確保やキー操作の簡素化といった効果がえられる」
		- 「ただし、正規化崩しにはやっかいな『報い』が伴う」

	- 論理フィールドの物理化
		- 図3-11
		- 「データモデルどおりに実装するとレスポンスが低下する」場合に実施する
		- 報い：「更新系ロジックが複雑化する」
	
	- サロゲートキーの導入

		- サロゲート＝代理
		- 「ある種の開発環境は複合主キーの利用を禁止しており、そういった環境を利用する場合には、この正規化崩しが必須」
		- 図3-13,3-14

	- テーブルの統合と抽象化
		- 統合
			- 図3-15
			- 共通する属性項目を持つテーブルを統合する
			- 報い：妥当性検査が仕様化されなければならない
		- 抽象化
			- 図3-16
			- 「大きな効果がある」⇒第5章以降で説明？
...
- データモデリングの練習問題
	...
	- 問題1
		- ｛｝で囲われ下線が引かれている項目が「主キー」
		- 主キーはテーブルの中で一意であるという特徴をもつ
		- 主キーが決まれば他の項目も特定できる → 関数従属性を持っているといえる

	- 問題2
		- 主キーでない項目が2つ以上ある場合は、それぞれの項目が主キーと関数従属している
	...
	- 問題4,5
		- 共通する主キーを持つのが「親子関係」
		- 一方のテーブルの属性項目(主キーではない項目)が、もう一方のテーブルの主キーになっているのが「参照関係」

	- 問題6,7
		- {a}->b,{b}->cのとき、{a}->cが推移的関数従属性
		- よくわからなかった
	
	- 問題8,9
		- 論理フィールドを使って成立している参照関係が「動的参照関係」

	- 問題10,11,12
		- まず、主キーと属性項目をまとめてテーブルを作る
		- 各テーブルの主キーを見比べて親子関係を見つける
		- 各テーブルの主キーと属性を見比べて参照関係を見つける
...
- コラム２　データモデルとシステム要件
	...
	- データモデルは「RDBの標準機能がカバーできる範囲を超えています」
	- 「ただし、いずれの選択肢を用いるとしてもデータモデリングをなおざりにしてはいけません」
	- 「保持されるデータに矛盾が生じるようなデータ構造を基礎とすればデータ管理は失敗するからです」
